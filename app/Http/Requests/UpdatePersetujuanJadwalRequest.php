...
class UpdatePersetujuanJadwalRequest extends FormRequest
{
    /**
    ...
    {
        $persetujuanId = $this->route('persetujuan_jadwal')->id;

        return [
            'jadwal_id' => ['sometimes', 'required', 'exists:jadwals,id'],
            'aktor_id' => ['sometimes', 'required', 'exists:users,id'],
            'status' => ['sometimes', 'required', 'string', 'in:MENUNGGU,DISETUJUI,DITOLAK'],
